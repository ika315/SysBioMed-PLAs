# -------------------------------------------------------------------
# 04_Benchmarking.R
# Benchmarking von PLA-Methoden anhand B-Zell Signatur
# Für Vision
# -------------------------------------------------------------------
...
#PATH_DATA <- "~/SysBioMed-PLAs/data/pbmc3k.final"
#DATASET_NAME <- "pbmc3k.final"
...
gt_col_name <- "seurat_annotations"
...
path_gene_list = "~/SysBioMed-PLAs/data/b_cells.csv"
...
pbmc$GT_Response <- ifelse(pbmc[[gt_col_name]] == "B", 1, 0)
...
pbmc$Vision_Raw <- vision_score

# UMAP
png(filename = "plots/counts_matrix/04_Vision_B_UMAP_seurat_annotation.png", width = 900, height = 700)
p_umap <- FeaturePlot(pbmc, features = "Vision_Raw", reduction = "umap", label = TRUE, label.size = 6, repel = TRUE, raster = TRUE) + 
  ...
  labs(title = "Vision Score (Memory B) auf UMAP", 
       subtitle = "Farbskala: Gelb = Hoch, Dunkelblau = Niedrig | Legende zeigt Seurat-Annotations-Klassen",
       color = "Vision Score") + 
...
# Violin Plot
png(filename = "plots/counts_matrix/04_Vision_B_Violin_seurat_annotation.png", width = 1000, height = 600)
p_l1 <- VlnPlot(pbmc, features = "Vision_Raw", group.by = "seurat_annotations", pt.size = 0) + # replace seurat_annotations by celltype.l2
  labs(title = "Vision Score Verteilung über Seurat-Annotationen") +
...
png(filename = "plots/counts_matrix/04_Vision_B_PrecisionRecall_Curve.png", width = 800, height = 700)
...
png(filename = "plots/counts_matrix/04_Vision_B_ZScore_Distribution.png", width = 1500, height = 800)
...
print("Confusion Matrix (Positive = B):")
...
pbmc$ZScore_Groups <- case_when(
  pbmc$seurat_annotations == "B" ~ "B",
  TRUE ~ "Other (Non-B)"
)

pbmc$ZScore_Groups <- factor(pbmc$ZScore_Groups, 
                             levels = c("B", "Rest B-cells", "Other (Non-B)"))

png(filename = "plots/counts_matrix/04_Vision_B_ZScore_3Groups.png", width = 800, height = 600)
p_z3 <- ggplot(pbmc@meta.data, aes(x = ZScore_Groups, y = Vision_ZScore, fill = ZScore_Groups)) +
  ...
  scale_fill_manual(values = c("B" = "#FF4B4B", "Rest B-cells" = "#4B8BFF", "Other (Non-B)" = "#BEBEBE")) +
...
png(filename = paste0("plots/counts_matrix/04_Vision_Bench_ErrorTypes_B.png"), width = 900, height = 650)
