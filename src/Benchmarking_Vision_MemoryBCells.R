# -------------------------------------------------------------------
# 04_Benchmarking.R
# Benchmarking von PLA-Methoden anhand B memory-Zell Signatur
# Für Vision
# -------------------------------------------------------------------
...
path_gene_list = "~/SysBioMed-PLAs/data/memory_b_cells.csv"
source("~/SysBioMed-PLAs/src/read_and_extend_gene_list.R")
genes <- read_gene_list(path_gene_list)
sig_vision <- make_signature(genes = genes, sig_name = "Memory_B_Cell_Signature", method = "vision")
...
vision_scores <- getSignatureScores(vision_obj)[, "Memory_B_Cell_Signature"]

score_name <- "Vision_Memory_B_Cell_Signature"
...
## ------------------------------------------------
message("=== Vision Benchmarking Metrics (Memory B) ===")
...
pbmc_out_path <- file.path(
  "results",
  paste0("pbmc_", DATASET_NAME, "_Vision_MemoryB.rds")
)
...
tryCatch({
  ...
  for (ann in annotation_levels) {
    ...
    png(
      filename = file.path("plots", "benchmarking_vision", paste0("04_Reference_UMAP_BMemory_", ann, ".png")),
      width = 1200,
      height = 800
    )
  ...
  for (ann in annotation_levels) {
    ...
    ## ---- UMAP (VISION SCORE) ----
    png(
      filename = file.path("plots", "benchmarking_vision", paste0("Vision_MemoryB_UMAP_", ann, ".png")),
    ...
    print(
      FeaturePlot(
        ...
        labs(
          title = "VISION Score (Memory B) auf UMAP",
    ...
    ## ---- VIOLIN ----
    png(
      filename = file.path("plots", "benchmarking_vision", paste0("Vision_MemoryB_Violin_", ann, ".png")),
  ...
  png("plots/benchmarking_vision/Vision_MemoryB_PrecisionRecall_Curve.png", 800, 700)
  print(p_pr)
  dev.off()
  ...
  png("plots/benchmarking_vision/Vision_MemoryB_ZScore_Distribution.png", 1500, 800)
  print(p_z)
  dev.off()
  ...
  png("plots/benchmarking_vision/Vision_MemoryB_ZScore_3Groups.png", 800, 600)
  print(p_z3)
  dev.off()
  ...
  png("plots/benchmarking_vision/04_Bench_ErrorTypes_MemoryB.png", 900, 650)
  print(p_error)
  dev.off()
  
  png("plots/benchmarking_vision/04_Vision_Bench_ErrorTypes_MemoryB.png", 900, 650)
  print(p_error)
  dev.off()
  ...
  ggsave(
    "plots/benchmarking_vision/04_Vision_ZScore_MemoryB_Facetted_TP_FP_FN_TN.png",
    faceted_z, width = 10, height = 6, dpi = 300
  )
  
  ggsave(
    "plots/benchmarking_vision/04_Vision_Raw_MemoryB_Facetted_TP_FP_FN_TN.png",
