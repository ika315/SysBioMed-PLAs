...
#GT and Signature Genes for Platelets
...
print("--- Berechne UCell (Original) für Gen-Erweiterung ---")
orig_sig <- list(Platelet_Orig = genes)
pbmc <- AddModuleScore_UCell(pbmc, features = orig_sig, name = NULL) 
pbmc$UCell_Raw_Original <- pbmc$Platelet_Orig

# Gen-Set erweiterung
message("=== Erweitere Platelet-Gen-Set basierend auf UCell Scores ===")
res_ext <- extend_gene_set(
  pbmc          = pbmc,
  base_genes    = genes,
  score_name    = "UCell_Raw_Original", 
  top_n         = 50,
  high_quantile = 0.9,
  min.pct       = 0.05,
  test.use      = "wilcox"
...
write.csv(data.frame(geneName = extended_genes), 
          "results/extended_platelet_gene_list_UCell.csv", row.names = FALSE, quote = FALSE)

print(paste("Originale Gene:", length(genes)))
print(paste("Erweiterte Gene:", length(extended_genes)))

# Final Scoring
print("--- Berechne finales UCell Scoring (Extended) ---")
pbmc <- AddModuleScore_UCell(pbmc, features = gene_set_extended, name = NULL, assay = "RNA")
